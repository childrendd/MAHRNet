...
class cSE(nn.Module):
    def __init__(self, inplanes):
        ...
        self.Conv_Squeeze = nn.Conv2d(inplanes, inplanes // 2, kernel_size=1, bias=False)
        self.Conv_Excitation = nn.Conv2d(inplanes//2, inplanes, kernel_size=1, bias=False)
        self.norm = nn.Sigmoid()
...
class BasicBlock(nn.Module):  #定义了一个 ResNet 中的 BasicBlock 模块,在网络深度较浅的情况下使用，以减少模型的计算量和参数数量
    expansion = 1
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        # self.aspp = ASPP(planes)
        # self.sc = scSE(planes)
        #self.arm = ARM(planes)
        # self.sp = Sp(planes)
        self.eca = ECA(planes)
        # self.non = NonLocalNet(planes)
        # self.gc = GC(planes)
        # self.se = SE(planes)
        # self.ca = CA(planes)
    def forward(self, x):
...
class Bottleneck(nn.Module): #定义了一个 ResNet 中的 Bottleneck 模块，用于构建深度神经网络。Bottleneck 模块是 ResNet 中用于减少参数数量和计算量
    expansion = 4
...
def hrnet_classification(pretrained=False, backbone='hrnetv2_w18'):
    model = HighResolutionNet_Classification(num_classes=2, backbone=backbone)
    if pretrained:
        model_urls = {
